...
import FollowedUserBtn from "./FollowedUserBtn";
...
const Users = (props) => {
  ...
  return (
    <div className={classes.users_container}>
      {
        props.usersPageData.users.map((user) => <div key={user.id} className={classes.users_item}>


          <AvatarPreview avatar={user.avatar}/>

          <div className={classes.users_item_name}>
            <UserName name={user.name}/>
            <Status status={user.status} />
          </div>


          <div className={classes.users_item_data}>
            <Location city={user.location.city} country={user.location.country} />

            {user.followed
              ? <FollowedUserBtn id={user.id} callback={props.unfollow} btnName={'Unfollow'}/>
              : <FollowedUserBtn id={user.id} callback={props.follow} btnName={'Follow'}/>
            }
          </div>
